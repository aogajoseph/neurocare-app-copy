// app/(drawer)/(content)/learn/myths/[slug].tsx
...
type UIBlock =
  ...
  | { type: 'related'; title: Localized; items: { slug: string; label: Localized }[] }
...
function buildBlocks(sections: any, lang: 'en' | 'sw'): UIBlock[] {
  ...
  if (sections.whyItMatters) {
    blocks.push({ type: 'text', title: sections.whyItMatters.title, content: sections.whyItMatters.content });
  }
  if (sections.tips) {
    blocks.push({ type: 'bullets', title: sections.tips.title, items: sections.tips.bullets?.[lang] ?? [] });
  }
  if (sections.relatedTopics?.length) {
    blocks.push({
      type: 'related',
      title: { en: 'Related Myths', sw: 'Hadithi Zinazohusiana' },
      items: sections.relatedTopics,
    });
  }
  if (sections.reassurance) {
    blocks.push({ type: 'reassurance', content: sections.reassurance });
  }
...
/* ──────────────────────────────
   SCREEN
────────────────────────────── */
export default function MythsDetailScreen() {
